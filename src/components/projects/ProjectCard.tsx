...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <Card className="flex flex-col bg-[#18181b] border-border/50 hover:border-border transition-colors">
      <CardHeader>
      ...
      <CardContent className="flex-grow">
        <div className="flex flex-wrap gap-2 mb-6">
          {project.technologies.slice(0, 5).map((tech) => (
            <Badge key={tech} variant="secondary" className="bg-zinc-800 text-zinc-300 hover:bg-zinc-700">
              {tech}
            </Badge>
          ))}
      ...
      <CardFooter className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-border/50 mt-auto">
